...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".Bujjamma_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".Bujjamma_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using Bujjamma profile")
                Bujjamma = str(Path.home() / ".Bujjamma_profiles" / "firefox_Bujjamma")
                Path(Bujjamma).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(Bujjamma, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".Bujjamma_profiles" / "safari")
        ...
        Bujjamma_profile = str(Path.home() / ".Bujjamma_profiles" / self.browser_name)
        Path(Bujjamma_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with Bujjamma profile: {Bujjamma_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(Bujjamma_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with Bujjamma profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "Bujjamma_screenshot.png")
