...
def _scripts_dir() -> Path:
    d = Path.home() / ".Bujjamma" / "reminders"
...
try:
    ...
    notification.notify(title="Bujjamma Reminder", message=message, timeout=15)
    notified = True
except Exception:
    pass
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("Bujjamma Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception:
        pass
...
if not notified:
    try:
        import subprocess
        script = 'display notification "{{0}}" with title "Bujjamma Reminder"'.format(
            message.replace('"', '')
        )
...
"""
    ...
    script_body = f"""# Auto-generated by Bujjamma reminder — do not edit
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    xml_content = (
        ...
        '  <RegistrationInfo><Description>Bujjamma Reminder</Description></RegistrationInfo>\n'
...
def _schedule_mac(target_dt: datetime, task_name: str,
    ...
    label     = f"com.Bujjamma.reminder.{task_name}"
...
def reminder(
    ...
    task_name  = f"BujjammaReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"
